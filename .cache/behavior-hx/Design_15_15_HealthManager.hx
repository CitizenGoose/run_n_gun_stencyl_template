...
class Design_15_15_HealthManager extends ActorScript
{
	...
	public function _customBlock_Damage(__Amount:Float):Void
	{
		...
		_CurrentHealth = Math.max((_CurrentHealth - __Amount), 0);
	...
	public function _customBlock_Heal(__Amount:Float):Void
	{
		...
		_CurrentHealth = Math.min((_CurrentHealth + __Amount), _MaximumHealth);
		_customEvent_UpdateGlobalHealth();
	...
	{
		var __Self:Actor = actor;
		_CurrentHealth = Math.min(__Amount, _MaximumHealth);
		_CurrentHealth = Math.max(__Amount, 0);
		_customEvent_UpdateGlobalHealth();
	}
	...
	override public function init()
	{
		...
		if(_PersistentHealth)
		{
			if(!(((asNumber(((getGameAttribute(_HealthIdentifier)) : Dynamic)) <= 0) || (asNumber(((getGameAttribute(_HealthIdentifier)) : Dynamic)) >= 0))))
			{
				setGameAttribute(_HealthIdentifier, asNumber(_CurrentHealth));
			}
			else
			{
				_CurrentHealth = asNumber(((getGameAttribute(_HealthIdentifier)) : Dynamic));
			}
		}
		if((_CurrentHealth <= 0))
		{
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				_PercentLeft = (_CurrentHealth / _MaximumHealth);
			}
		});
		...
		addWhenDrawingListener(null, function(g:G, x:Float, y:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				if(_DrawHealthBar)
				{
					...
					{
						g.fillColor = Utils.convertColor(_HealthBarOutlineColour);
					...
					g.fillColor = Utils.convertColor(_HealthBarBackgroundColour);
					g.fillRect(_HealthBarXOffset, _HealthBarYOffset, _HealthBarWidth, _HealthBarHeight);
					if((_PercentLeft > 0.75))
					{
						g.fillColor = Utils.convertColor(_ColourOver75);
					}
					else if((_PercentLeft > 0.50))
					{
						g.fillColor = Utils.convertColor(_ColourOver50);
					}
					else if((_PercentLeft > 0.25))
					{
						g.fillColor = Utils.convertColor(_ColourOver25);
					}
					else
					{
						g.fillColor = Utils.convertColor(_ColourUnder25);
					}
