...
class ActorEvents_1 extends ActorScript
{
	...
	override public function init()
	{
		...
		addCollisionListener(actor, function(event:Collision, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled && sameAsAny(getActorType(7), event.otherActor.getType(),event.otherActor.getGroup()))
			{
				if((event.otherActor.getAnimation() == "enemy"))
				{
					...
					Engine.engine.setGameAttribute("HP", ((Engine.engine.getGameAttribute("HP") : Float) - 10));
				}
			}
		});
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				if(isKeyDown("slowmoo"))
				{
					if(((Engine.engine.getGameAttribute("timeFlow") : Float) == 10))
					{
						Engine.STEP_SIZE = 50;
						Engine.engine.setGameAttribute("timeFlow", 50);
					}
				}
				else
				{
					...
					{
						Engine.STEP_SIZE = 10;
						Engine.engine.setGameAttribute("timeFlow", 10);
					}
				}
			}
		});
