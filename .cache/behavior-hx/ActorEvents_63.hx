			...
			if(wrapper.enabled)
			{
				if(((Engine.engine.getGameAttribute("timeFlow") : Float) == 10))
				{
					if((_mode == "attack"))
					{
						if(actor.isOnScreen())
						{
							createRecycledActor(getActorType(7), (actor.getXCenter() + 60), (actor.getYCenter() + 0), Script.FRONT);
							actor.setAnimation("enemy");
							getLastCreatedActor().applyImpulseInDirection((Utils.DEG * actor.getAngle()), 100);
							_DistanceX = ((getScreenX() + _player.getXCenter()) - getLastCreatedActor().getXCenter());
							_DistanceY = ((getScreenY() + _player.getYCenter()) - getLastCreatedActor().getYCenter());
